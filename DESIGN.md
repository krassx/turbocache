...
17. **Dead-code removal, partly done.** The background-compaction machinery is
    gone (decision 47) and the test-only hooks are off the public surface
    (decision 45). Two rejected alternatives are still compiled in, each kept
    alive only by its own bench: the `SLAB` allocator that decision 16 did not
    choose -- reachable as arena mode 0, still exercised by `test/test.js` and
    the mode-comparison benches, and costing three `mode != MODE_SLAB` branches
    on the read path -- and the native size estimator that lost to
    `encodedBytes x 3` (`estimateSize`, used only by `bench/sizing.js`).
    Removing `SLAB` would also retire arena mode 1 (`LOG`), since the shipped
    JS layer hardcodes mode 2.
